...
var file = parent.file;
	    if(file.files&&file.files[0]){
	      ...
	      reder.onload = function(evt){
	          $(".img-container").children("img").attr("src",evt.target.result) ;
	    	  zhixing();
	    	  parent.$("#coverupload").get(0).value='';
	      } 
	    reder.readAsDataURL(file.files[0]);
	   }else{
		   $(".img-container").children("img").html('<div class="img" style="filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=scale,src=\''+file.value+'\'"></div>');
	}    
	    ...
	    function zhixing() {
		   ...
		   var cutafter = parent.$('#getCroppedCanvasModal')
		   var ptips  = parent.$(".covertips");
		    // Methods
		    $(document.body).on('click', '[data-method]', function () {
		      ...
		      if (data.method) {
		        ...
		        if (data.method === 'getCroppedCanvas') {
		        	cutafter.find('.modal-body').html(result);
		  	...
		  	cutafter.css("display","block");
        	ptips.css("display","none");
